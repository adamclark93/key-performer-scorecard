...
import { buildDay14Email } from "../../src/email/day14Template.js";
    ...
    for (const lead of leads || []) {
      ...
      try {
        await resend.emails.send({
          ...
          to: recipient,
          subject: "Two weeks on — let's talk",
          html: buildDay14Email(userData, result),
        });

        if (!testEmail) {
          await supabase
            .from("leads")
            .update({ day14_sent_at: new Date().toISOString() })
            .eq("id", lead.id);
        }
